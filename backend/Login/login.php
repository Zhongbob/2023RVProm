...
require_once "googleLoginInfo.inc.php";
...
if (isset($_GET['code'])) {
    ...
    if ($token["error"])
        echo "<script>window.alert('Error logging in! Please contact rdevcca@gmail.com for assistance.'); window.location.href = '../../index.php' </script>";
    ...
    if ($email) {
        ...
        $fromrvhs = ($google_info['hd'] == "students.edu.sg" && $fromrvhs) || $google_info['hd'] == "moe.edu.sg";
        ...
        if ($exists) {
            ...
            $sql = "INSERT INTO users (`email`, `username`,`fromrvhs`) VALUES (?, ?, ?)";
            $stmt = prepared_query($conn, $sql, [$email, $name, $fromrvhs], "ssi");
            mysqli_stmt_close($stmt);
